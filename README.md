...
一个动物森友会风格的**在线记账应用**：记录每一笔收支、做预算、立存钱目标、统计图表、达成成就。Vite + React 前端 + Express + MySQL 后端，支持多账号注册登录与数据隔离。

## ✨ 功能特性

- **记一笔**：支持输入 金额、分类、账户、日期、**时间、地点、支付方式、详细说明**（支出 / 收入通用，均为选填），首页与弹窗共用同一表单，可编辑 / 删除 / 快捷连记
- **流水明细**：按日分组展示，支持按 类型 / 分类 / 账户 / **日期区间** 筛选和关键词搜索（命中说明、地点、支付方式），分页展示
- **统计页**：日期区间收入/支出、分类/大类占比饼图、每日收支折线图、年度每月柱状图，报表可复制/导出
- **预算**：分类预算（总预算 = 各分类预算合计），超支红色警示
- **账户**：现金 / 银行卡 / 支付宝 / 微信 等钱包，初始余额 + 收支推导当前余额，支持**账户间转账**
- **存钱目标**：立目标、存一笔、倒计时、动森风格进度条
- **成就徽章**：数据驱动，完成记账 / 连续记账等行为解锁
- **设置**：岛主名字、大类/分类管理、数据导入导出备份、清空/示例数据
...
| 端 | 技术 |
|---|---|
| 前端 | Vite + React 18 + TypeScript + `animal-island-ui`（动森风组件库）+ react-router-dom（HashRouter）+ zustand（状态管理）+ recharts |
...
jizhang-app/
├─ server/
│  ├─ index.js        # Express + MySQL 后端（自动建库建表 + 幂等迁移 + 鉴权 + 审计日志）
│  ├─ package.json
...
后端 `server/index.js` **首次启动会自动建库建表**（`CREATE DATABASE IF NOT EXISTS jizhang` + 12 张表 `CREATE TABLE IF NOT EXISTS`），并执行幂等迁移：补 `user_id` 列、主键升级为 `(user_id, id)`、`categories` 补 `group_id` 列、`txs` 表补充时间/地点/支付方式列/`note` 扩为 `TEXT`、为存量账号初始化数据版本号 `version=1`。通常无需手动建库。
...
- **全局记账弹窗**由 `src/store/useUiStore.ts` 控制；登录后若服务端版本为 0 且账户列表为空时自动种入默认分类/账户（`useBookStore.init()` 双条件判断）
- 前端数据变更 → 防抖 200ms → `PUT /api/state` 全量提交；后端事务内先清后插

## ⚠️ 备注

- `animal-island-ui` 为 npm 依赖的动森风组件库（同级目录 `animal-island-ui-main` 是其源码）
- 后端纯 JS 不经 tsc 检查，修改时多留心 camelCase ↔ snake_case 字段名映射
